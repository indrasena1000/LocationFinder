...
//        mMap.moveCamera(CameraUpdateFactory.newLatLng(sydney));
   ...
   /* private GoogleMap mMap;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editTextLongitude = (EditText) findViewById(R.id.editText);
        editTextLatitude = (EditText) findViewById(R.id.editText2);
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
              ...
              String[] stringLat = databaseLatitude.split(",");
                  Arrays.sort(stringLat);
                  String latitude = stringLat[stringLat.length-1].split("=")[1];

                  String[] stringLon = databaseLongitude.split(",");
                  Arrays.sort(stringLon);
                  String longitude = stringLon[stringLon.length-1].split("=")[1];

                  LatLng latLng = new LatLng(Double.parseDouble(latitude),Double.parseDouble(longitude));

                  mMap.addMarker(new MarkerOptions().position(latLng).title(longitude +","+  latitude));
                  mMap.moveCamera(CameraUpdateFactory.newLatLng(latLng));
              } catch (Exception e){
                  e.printStackTrace();
              }
